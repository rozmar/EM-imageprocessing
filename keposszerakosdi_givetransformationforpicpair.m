function tforms=keposszerakosdi_givetransformationforpicpair(basepic,newpic,stdszorzo,h,hnagy,plotthestuff)
...
eddigeleg=2;
mostkezdodik=1;
while (((length(inlierbase)<13 &ebboleleg<eddigeleg| length(inlierbase)>100) & (400<=mthresh)) & length(inlierbase)<1000 & length(inlierbase)/length(matchedbase)<.9);
    if mostkezdodik==1
        mostkezdodik=0;
...
if ebboleleg==0 & length(inlierbase)>=13
    Tinv  = tform.invert.T;
