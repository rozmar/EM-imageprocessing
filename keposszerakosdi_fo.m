    ...
    if isempty(a) & length(idxes)>=2
        for picnum=1:length(idxes)-1
            for picnum2=picnum+1:length(idxes)
                ...
                tformsmatrix{picnum,picnum2}=keposszerakosdi_givetransformationforpicpair(basepic,newpic,stdszorzo,h,hnagy,plotthestuff);
            end
        end
% % % % %         %% regi megkozelites
% % % % %         locations=[0 0];
...
% % % % %                 if isstruct(tformsmatrix{picnum2,picnum})
% % % % %                     if isempty(locnow)
% % % % %                         NEXT=1;
% % % % %                     else
% % % % %                         NEXT=size(locnow,1)+1;
% % % % %                     end
% % % % %                     locnow(NEXT,:)=tformsmatrix{picnum2,picnum}.tform.T(3,1:2)+tformsmatrix{picnum2,picnum}.tform2.T(3,1:2)+ locations(picnum2,:);
% % % % %                 end
% % % % %             end
% % % % %             locations(picnum,:)=mean(locnow,1);
% % % % %         end
% % % % %         fname=filek(idxes(1)).name;
% % % % %         image=imread([dirs.tifkonyvtar,fname]);
% % % % %         locations=(bsxfun(@(x,y) x-y,locations,min(locations)))+1;
% % % % %         %          locations=(bsxfun(@(x,y) x+y,locations,size(image)));
% % % % %         locations=round(locations);
% % % % %         montage=keposszerakosdi_mergepicturesbyrelativelocations(dirs,filek,idxes,locations,hnagy,plotthestuff);
         ...
         for i=1:length(idxes)
          ...
          locations(1).ncoords=1;
           locations(1).hasacoord=true;
            ...
            locations(1).order=1;
            while sum([locations.hasacoord])<length(locations)
...
%                              disp('yaaay')
            ...
            for i=1:length(locations)
                loc(i,:)=mean(locations(i).coord,1);
            end
            locations=loc;
            fname=filek(idxes(1)).name;
        image=imread([dirs.tifkonyvtar,fname]);
...
for fnum=1:length(filek)
    ...
    filek(fnum).idx=str2num(fname(hyp(end)+1:end-4));
    if ~any(strfind(filek(fnum).ID,'X1200'))
        todel=[todel,fnum];
    end
end
filek(todel)=[];
...
for fnum=2:length(filek)
    ...
    if fnum==2
        ...
        backgrounddims=basepicdims*3;
        background=zeros(backgrounddims);
        background(basepicdims(1):basepicdims(1)*2-1,basepicdims(2):basepicdims(2)*2-1)=basepic;
        basepic=uint8(background);
    ...
    background=zeros(backgrounddims);
    background(newpicdims(1):newpicdims(1)*2-1,newpicdims(2):newpicdims(2)*2-1)=newpic;
    newpic=uint8(background);
    ...
    ptsbase  = detectSURFFeatures(temp1,'MetricThreshold',mthresh,'NumOctaves',2,'NumScaleLevels',15);
    ptsnew = detectSURFFeatures(temp2,'MetricThreshold',mthresh,'NumOctaves',2,'NumScaleLevels',15);
